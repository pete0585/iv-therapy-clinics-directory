          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
              <div className="flex items-start justify-between gap-4 flex-wrap mb-4">
                <div>
                  <div className="flex items-center gap-2 flex-wrap mb-2">
                    ...
                    {listing.listing_tier === 'featured' && (<span className="inline-flex items-center gap-1 px-2 py-0.5 text-xs font-semibold bg-amber-100 text-amber-800 rounded-full">
                        <Star className="w-3 h-3"/> Featured
                      </span>)}
                    {listing.listing_tier === 'verified' && (<span className="inline-flex items-center gap-1 px-2 py-0.5 text-xs font-semibold bg-teal-100 text-teal-800 rounded-full">
              ...
              <div className="flex flex-wrap gap-2">
                {listing.is_mobile && (<span className="px-2.5 py-1 text-xs font-medium bg-blue-50 text-blue-700 rounded-full">Mobile IV</span>)}
                {listing.medical_oversight && (<span className="px-2.5 py-1 text-xs font-medium bg-purple-50 text-purple-700 rounded-full">Medical Oversight</span>)}
              </div>
            </div>

            {isClaimed && listing.description && (<div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
            ...
            {listing.services_offered && listing.services_offered.length > 0 && (<div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
                <h2 className="text-lg font-bold text-gray-900 mb-3">IV Treatments Offered</h2>
                <div className="flex flex-wrap gap-2">
                  {listing.services_offered.map((s: string) => (<span key={s} className="px-3 py-1.5 text-sm bg-teal-50 text-teal-800 rounded-full">{s}</span>))}
                </div>
              </div>)}

            {!isClaimed && (<div className="bg-teal-50 border border-teal-200 rounded-xl p-5">
          ...
          <div className="space-y-4">
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
              ...
              {(<div className="space-y-3">
                  {listing.phone && (<a href={`tel:${listing.phone}`} className="w-full inline-flex items-center justify-center gap-2 px-4 py-3 bg-teal-600 text-white font-semibold rounded-xl hover:bg-teal-700 transition-colors">
                      <Phone className="w-5 h-5"/>
                      {formatPhone(listing.phone)}
                    </a>)}
                  {listing.website && (<a href={listing.website} target="_blank" rel="noopener noreferrer" className="w-full inline-flex items-center justify-center gap-2 px-4 py-3 bg-white border border-gray-300 text-gray-700 font-medium rounded-xl hover:border-teal-500 transition-colors">
            ...
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
              ...
              {listing.address && (<a href={`https://maps.google.com/?q=${encodeURIComponent(`${listing.name} ${listing.address ?? ''} ${listing.city} ${listing.state}`)}`} target="_blank" rel="noopener noreferrer" className="mt-3 inline-flex items-center gap-1.5 text-sm text-teal-600 hover:underline">
                  <MapPin className="w-4 h-4"/>
                  Get Directions
                </a>)}
            </div>

            {listing.hours_text && (<div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
