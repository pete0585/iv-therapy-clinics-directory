...
export default async function HomePage() {
    ...
    return (<div>
      ...
      <section className="py-12 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
            {FEATURED_TREATMENTS.map(({ slug, name, emoji, tagline }) => (<Link key={slug} href={`/iv-therapy/${slug}`} className="card p-4 text-center hover:border-brand-cyan hover:shadow-md transition-all group">
      ...
      {/* Featured Listings */}
      {featuredListings.length > 0 && (<section className="py-12 bg-brand-navy-light">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      {/* Browse by City */}
      {topCities.length > 0 && (<section className="py-12 bg-white">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
              {topCities.map(({ city, state, count }) => (<Link key={`${city}-${state}`} href={`/iv-therapy-clinics/${stateSlug(state)}/${citySlug(city)}`} className="card p-3 text-center hover:border-brand-cyan transition-all group">
      ...
      {/* Browse by State */}
      {stateList.length > 0 && (<section className="py-12 bg-gray-50">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
              {stateList.map(([state, count]) => (<Link key={state} href={`/iv-therapy-clinics/${stateSlug(state)}`} className="flex items-center justify-between p-3 rounded-xl border border-gray-200 bg-white hover:border-brand-cyan hover:bg-brand-cyan-light transition-all group">
      ...
      <section className="py-14 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <FeatureCard icon="💊" title="Treatment-Specific Search" description="Search by Myers Cocktail, NAD+, Glutathione, hangover recovery, and 10+ more treatments. Find clinics that offer exactly what you need."/>
            <FeatureCard icon="💰" title="Pricing Transparency" description="See price ranges before you call. Verified clinics display their session rates — no more calling five clinics to compare prices."/>
            <FeatureCard icon="🩺" title="Medical Oversight Level" description="Know if a clinic is MD-supervised, NP-led, or RN-administered before you book. The #1 trust question first-timers ask — answered upfront."/>
          </div>
        </div>
      </section>
      ...
      <section className="py-12 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {Object.entries(TREATMENT_SLUGS).map(([slug, name]) => (<Link key={slug} href={`/iv-therapy/${slug}`} className="flex items-center justify-between p-4 bg-white rounded-xl border border-gray-200 hover:border-brand-cyan transition-all group">
...
function SearchHero() {
    return (<form action="/iv-therapy-clinics" method="get" className="flex flex-col sm:flex-row gap-3">
      <div className="flex-1 relative">
        ...
        <input type="text" name="q" placeholder="City, state, or clinic name..." className="w-full pl-10 pr-4 py-4 rounded-xl bg-white text-gray-900 placeholder-gray-400 border-0 focus:outline-none focus:ring-2 focus:ring-brand-cyan text-base shadow-lg"/>
      </div>
      <button type="submit" className="px-8 py-4 bg-brand-amber text-white font-bold rounded-xl hover:bg-brand-amber-dark transition-colors text-base shadow-lg whitespace-nowrap">
        Find Clinics
      </button>
    </form>);
}
function QuickFilter({ href, emoji, label }: {
    href: string;
    emoji: string;
    label: string;
}) {
    return (<Link href={href} className="inline-flex items-center gap-2 px-4 py-2 bg-white/10 hover:bg-white/20 text-white rounded-full text-sm font-medium transition-colors border border-white/20">
      <span>{emoji}</span>
      {label}
    </Link>);
}
function ListingCard({ listing }: {
    listing: IvTherapyListing;
}) {
    const price = formatPriceRange(listing.price_low, listing.price_high);
    return (<Link href={`/iv-therapy-clinics/${stateSlug(listing.state)}/${citySlug(listing.city)}/${listing.slug}`} className="card p-5 block hover:border-brand-cyan">
      ...
      <div className="flex flex-wrap gap-1.5 mb-3">
        ...
        {listing.is_franchise && listing.franchise_name && (<span className="badge-franchise">{listing.franchise_name}</span>)}
        {listing.services_offered?.slice(0, 2).map((s) => (<span key={s} className="badge-treatment">{s}</span>))}
      </div>
      {price && (<div className="text-sm font-semibold text-brand-emerald mb-2">{price}</div>)}
      {listing.phone && (<div className="flex items-center gap-2 text-sm text-brand-cyan font-medium">
