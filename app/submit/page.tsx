          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <select id="state" {...register('state')} className="select-field">
                      ...
                      {US_STATES.map(([abbrev, name]) => (<option key={abbrev} value={abbrev}>{name}</option>))}
                    </select>
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-6">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <textarea id="description" {...register('description')} className="input-field" rows={4} placeholder="Tell patients about your clinic, specialties, and what makes you different..."/>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-6">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                {TREATMENT_CATEGORIES.map((treatment) => (<button key={treatment} type="button" onClick={() => toggleTreatment(treatment)} className={`px-3 py-2 rounded-lg text-xs font-medium text-left transition-colors border ${selectedTreatments.includes(treatment)
...
function PricingTier({ name, price, features, highlighted = false }: {
    ...
    return (<div className={`rounded-xl border p-5 ${highlighted ? 'border-brand-cyan bg-white shadow-md' : 'border-gray-200 bg-white'}`}>
      ...
      <ul className="space-y-1.5 mt-3">
        {features.map((f) => (<li key={f} className="flex items-start gap-2 text-xs text-gray-600">
